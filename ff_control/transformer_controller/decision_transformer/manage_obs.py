...
from optimization.ff_scenario_obs import safety_margin, robot_radius, table
...
def import_dataset_for_DT_eval_vXX(dataset_scenario, mdp_constr):
    ...
    if mdp_constr:
        ...
        data_param = {
            ...
            'n_obs': np.concatenate((data_param['n_obs'], data_param['n_obs']), axis=0)[perm],
            'obs_position': np.concatenate((data_param['obs_position'], data_param['obs_position']), axis=0)[perm],
            'obs_radius': np.concatenate((data_param['obs_radius'], data_param['obs_radius']), axis=0)[perm]
        }
...
def get_DT_model(model_name, train_loader, eval_loader, checkpoint_name=None):
    ...
    if checkpoint_name is None:
        ...
        accelerator.load_state(root_folder + '/decision_transformer/saved_files/' + checkpoint_name + "/" + model_name)

    return model.eval()


# def use_model_for_imitation_learning(model, test_loader, data_sample, rtg_perc=1., ctg_perc=1., rtg=None,
#                                      use_dynamics=True, output_attentions=False):
...
#
#     state_ol_t_unnorm = (states_ol[:, 0, :].to(device) * data_stats['states_std'][0].to(device)) + \
#                         data_stats['states_mean'][0].to(device)
...
#
#         state_dyn_t_unnorm = (states_dyn[:, 0, :].to(device) * data_stats['states_std'][0].to(device)) + \
#                              data_stats['states_mean'][0].to(device)
...
#         action_ol_t_unnorm = (action_ol_t.to(device) * (data_stats['actions_std'][t].to(device) + 1e-6)) + \
#                              data_stats['actions_mean'][t].to(device)
...
#             state_ol_t_unnorm = (state_ol_t.to(device) * (data_stats['states_std'][t + 1].to(device) + 1e-6)) + \
#                                 data_stats['states_mean'][t + 1].to(device)
...
#             attention_mask_ol = torch.cat((attention_mask_ol, attention_mask_i[:, t + 1][None, :].to(device)),
#                                           dim=1).long()
...
#             action_dyn_t_unnorm = (action_dyn_t.to(device) * (data_stats['actions_std'][t].to(device) + 1e-6)) + \
#                                   data_stats['actions_mean'][t].to(device)
...
#                 states_dyn_norm = (torch.tensor(xypsi_dyn[:, t + 1]).to(device) - data_stats['states_mean'][t + 1].to(
#                     device)) / (data_stats['states_std'][t + 1].to(device) + 1e-6)
...
#                 attention_mask_dyn = torch.cat((attention_mask_dyn, attention_mask_i[:, t + 1][None, :].to(device)),
#                                                dim=1).long()
...
def plot_DT_trajectory(DT_trajectory, plot_orb_time=False, savefig=False, plot_dir=''):
    ...
    ax1.add_patch(
        Rectangle((0, 0), table['xy_up'][0], table['xy_up'][1], fc=(0.5, 0.5, 0.5, 0.2), ec='k', label='table',
                  zorder=2.5))
    for n_obs in range(obs['radius'].shape[0]):
        ...
        ax1.add_patch(
            Circle(obs['position'][n_obs, :], obs['radius'][n_obs] + robot_radius, fc='r', alpha=0.2, label=label_robot,
                   zorder=2.5))
