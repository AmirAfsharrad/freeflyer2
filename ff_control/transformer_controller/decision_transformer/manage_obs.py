...
def import_dataset_for_DT_eval_vXX(dataset_scenario, mdp_constr):
    ...
    if mdp_constr:
        ...
        torch_observations = torch.cat((observations_scp, observations_cvx), axis=0)[perm]
...
#     dt = dt.item()
#     time_sec = np.array(time_sec[0])
#     obs_pos, obs_rad = torch.tensor(np.copy(obs['position'])).to(device), torch.tensor(np.copy(obs['radius'])).to(
#         device)
#     obs_rad = (obs_rad + robot_radius) * safety_margin
#     ff_model = FreeflyerModel()
